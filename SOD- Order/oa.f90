...
MODULE subroutines
    ...
    SUBROUTINE init_density_wave(x,rho,ux,p)
        IMPLICIT NONE
      ...
      ! Density wave
        do i = 1,nx
          ux(i)  = 0.1d0
          p(i)   = 20d0  
          rho(i) = 1.0d0 + 0.98d0 * sin(2* pi * (x(i)))
        end do
      
        END SUBROUTINE init_density_wave
    ...
    SUBROUTINE ApplyBoundaryConditions(rho, ux, p)
        ...
        ! ============================
        ! We have ng ghost cells on each side, total size = nx,
        ! so the physical domain is i = ng+1 : nx-ng.
    
        ! Left boundary: copy from the right side
        DO i = 1, ng
            rho(i) = rho(nx - 2*ng + i)
            ux(i)  = ux(nx - 2*ng + i)
            p(i)   = p(nx - 2*ng + i)
        ...
        ! Right boundary: copy from the left side
        DO i = 1, ng
            rho(nx - ng + i) = rho(ng + i)
            ux(nx - ng + i)  = ux(ng + i)
            p(nx - ng + i)   = p(ng + i)
        END DO
    ...
    !**************************************************************
    SUBROUTINE L1_norm(x, rho, l1)
        IMPLICIT NONE
        REAL(dp), INTENT(IN) :: x(:), rho(:)
        REAL(dp), INTENT(OUT) :: l1
        REAL(dp), ALLOCATABLE :: rho_exact(:)
        INTEGER i
    
        ALLOCATE(rho_exact(nx))
    ...
    !**************************************************************
    SUBROUTINE exact_solution(rho_exact, x)
        IMPLICIT NONE
        REAL(DP), INTENT(IN), DIMENSION(:)  :: x    
        REAL(DP), INTENT(OUT), DIMENSION(:) :: rho_exact
    ...
    !**************************************************************
    SUBROUTINE output(scheme, grid_size, l1)
        IMPLICIT NONE
        INTEGER i
    ...
    !**************************************************************
    SUBROUTINE solve(x, rho, u_x, p, eu, cfl, ct, sharpness, c, fh_x, scheme, solver, l1)
        IMPLICIT NONE
        INTEGER                             ,INTENT(IN)     :: ct
        REAL(DP)                            ,INTENT(OUT)     :: cfl, sharpness, l1
        ...
        CALL grid(x)
        CALL init_density_wave(x, rho, u_x, p)
        CALL solvec(rho, u_x, p, eu)
    ...
    PROGRAM myeuler1d 
        ...
        REAL(DP), DIMENSION(:), ALLOCATABLE     :: x, rho, u_x, p, c, l1
        REAL(DP), DIMENSION(:,:), ALLOCATABLE   :: eu, fh_x
        REAL(DP)                                :: betas(1) = (/2.d0/)
        INTEGER                                 :: cts(1) = (/(-5)/), ct
        ...
        DO  scheme_index = 1, SIZE(scheme)
            ...
            DO  grid_index = 1, SIZE(grid_size)
                ...
                tfinal = 2.0
                sharpness = 2.d0
                CALL solve(x, rho, u_x, p, eu, cfl, cts(1), sharpness, c, fh_x,&
                scheme(scheme_index), solver(1), l1(grid_index))
            END DO
            CALL output(scheme(scheme_index), grid_size, l1)
            DEALLOCATE(l1)
        END DO
