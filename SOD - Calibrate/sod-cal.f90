...
MODULE subroutines
    ...
    !**************************************************************
    SUBROUTINE output(x, rho, u_x, p, scheme, solver, ct, beta)
                    IMPLICIT NONE
                    INTEGER i
                    CHARACTER(LEN=50)            :: filename
                    CHARACTER(LEN=4), INTENT(IN) :: scheme, solver
                    INTEGER         , INTENT(IN) :: ct
                    REAL(DP)        , INTENT(IN) :: x(:), rho(:), u_x(:), p(:), beta
                    
                    ! Include the swept parameter in the filename for each calibration run.
                    IF (scheme == "MUSC") THEN
                        WRITE(filename, '(A,A,A,A,F3.1,A)') scheme,"_",solver,"_beta_", beta, ".dat"
                    ELSE IF (scheme == "TENO") THEN
                        WRITE(filename, '(A,A,A,A,I0,A)') scheme,"_",solver,"_ct_", ct, ".dat"
                        print *, ct
                    END IF
    ...
    !**************************************************************
    SUBROUTINE solve(x, rho, u_x, p, eu, cfl, ct, sharpness, c, fh_x, scheme, solver)
        IMPLICIT NONE
        INTEGER                             ,INTENT(IN)     :: ct
        REAL(DP)                            ,INTENT(IN)     :: cfl, sharpness
        REAL(DP),ALLOCATABLE, DIMENSION(:)  ,INTENT(INOUT)  :: x, rho, u_x, p, c
        REAL(DP),ALLOCATABLE, DIMENSION(:,:), INTENT(INOUT) :: eu, fh_x
        CHARACTER(LEN=4)                    , INTENT(IN)    :: scheme, solver

        
        ! Allocate, initialize, solve, write, and release one calibration case.
        ...
        CALL RK3TVD(eu, cfl, 10.d0**(ct), sharpness, rho, u_x, p, c, fh_x, &
         scheme, solver)
        CALL output(x, rho, u_x, p, scheme, solver, ct, sharpness)
    ...
    PROGRAM myeuler1d 
                ...
                IMPLICIT NONE
                INTEGER                                 :: scheme_index, solver_index, grid_index, sharpness_index, ct_index
                REAL(DP)                                :: cfl, sharpness
                REAL(DP), DIMENSION(:), ALLOCATABLE     :: x, rho, u_x, p, c
                REAL(DP), DIMENSION(:,:), ALLOCATABLE   :: eu, fh_x
                REAL(DP)                                :: betas(4) = (/1.6d0, 2.d0, 2.4d0, 2.8d0/)
                INTEGER                                 :: cts(4) = (/(-2), (-4), (-5), (-8)/), ct
                INTEGER                                 :: grid_size(1) = (/800/)
                CHARACTER(LEN=4)                        :: scheme(1) = (/"TENO"/), solver(1) = (/"HLLC"/)
                
                ! Driver for the Sod parameter sweep used to tune TENO and MUSCL-THINC.
                ...
                DO grid_index = 1, SIZE(grid_size)
                    ...
                    ! Run TENO and MUSCL schemes.
                    DO scheme_index = 1, SIZE(scheme)
                        DO solver_index = 1, SIZE(solver)
                            ! Study sharpness in MUSCLE
                            IF (scheme(scheme_index) == "MUSC") THEN
                                DO sharpness_index = 1, SIZE(betas)
                                    CALL solve(x, rho, u_x, p, eu, cfl, ct, betas(sharpness_index), c, fh_x, scheme(scheme_index),&
                                     solver(solver_index))
                                END DO
                            ! Study ct in TENO
                            ELSE IF (scheme(scheme_index) == "TENO") THEN
                                DO ct_index = 1, SIZE(cts)
                                    CALL solve(x, rho, u_x, p, eu, cfl, cts(ct_index), sharpness, c, fh_x,&
